...
import {GlobalTheme} from "naive-ui/es/config-provider/src/interface";
...
const base = {
  ...
  // primary
  primaryHover: '#b73fb2',
  primaryDefault: '#bb15b3',
  primaryActive: '#7c0776',
  primarySuppl: '#be31b8',
  primarySelected: '#d38bd0',
...
const commonPalace = {
  ...
  primarySelected: base.primarySelected,
  // info color
...
const palaceTheme: GlobalTheme = {
  name: 'forest',
  common: commonPalace,
}

Reflect.ownKeys(lightTheme).forEach(component => {
  palaceTheme[component] = {
    name: lightTheme[component].name,
    common: commonPalace,
    self: lightTheme[component].self,
  }
})

Reflect.ownKeys(lightTheme).forEach(component => {
  if (!lightTheme[component].peers) return

  palaceTheme[component].peers = {}

  Reflect.ownKeys(lightTheme[component].peers).forEach(peerComponent => {
    palaceTheme[component].peers[peerComponent] = palaceTheme[peerComponent]
  })
})



export default palaceTheme